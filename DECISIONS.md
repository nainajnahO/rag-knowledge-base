...
2. **`ts_rank_cd`, not `ts_rank`.** Cover-density variant — proximity-aware. Free upgrade and what both canonical examples use.
...
**Alternatives considered:**
- **No reranker** (the previous §15 deferred-cut framing — "RRF gets us most of the way there"). Anthropic's *Introducing Contextual Retrieval* post directly contradicts this: contextual hybrid alone reduced top-20 retrieval failure by 49%, **adding a reranker took it to 67%**. Most of the remaining quality lives in the reranker, not in RRF tuning.
- **Cohere `rerank-3.5`** or other cross-encoders. Voyage is Anthropic's recommended embedding/reranking partner — same vendor as `voyage-4`, same SDK, same error class hierarchy reused in `app/rerank.py`. Switching would split the upstream story without quality justification.
...
**Why this choice:**
- Matches Anthropic's canonical 2026 RAG pipeline: hybrid retrieve → rerank → top-K to LLM. *Introducing Contextual Retrieval* (anthropic.com/news/contextual-retrieval) describes exactly this shape and reports the 49%→67% number above.
...
3. **Structural empty-retrieval guard.** Lives in the route layer: if `retrieve()` followed by `rerank()` returns zero candidates, return the refusal response with `sources: []` without calling Anthropic. This fires when metadata filters reject every chunk or the corpus is empty — there's literally nothing to send. **No score threshold.** The previous gate (cosine ≥ 0.5, dropped in Step 7) does not survive contact with the canonical 2026 hybrid+rerank pipeline: Anthropic's *Introducing Contextual Retrieval* post, pgvector's RRF example, and Supabase's `hybrid_search` RPC all take top-K unconditionally; none threshold rerank scores. Voyage doesn't document `relevance_score` as a calibrated `[0,1]` quantity, so picking a fixed cutoff would tune against an uncalibrated, query-dependent signal. Refusal on low-relevance retrieval is owned by Claude's response to guardrails #1 and #2 — that's the canonical pattern.
