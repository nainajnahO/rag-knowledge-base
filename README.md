...
| **Hybrid search + rerank** | RRF (`k=60`, 50 candidates per lane, `FULL OUTER JOIN`, `ts_rank_cd`, `'simple'` tsv config) → Voyage `rerank-2.5` → top-K. | RRF is parameter-free and no score normalization is needed; SQL shape mirrors pgvector's and Supabase's canonical examples. Reranker matches Anthropic's *Introducing Contextual Retrieval* recipe — most of the remaining quality gain after hybrid lives in the reranker. ([§7](./DECISIONS.md#7-hybrid-search-fusion) / [§7.2](./DECISIONS.md#72-reranker--voyage-rerank-25)) |
